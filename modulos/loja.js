/* ============================================================
   economizei-loja.js
   Módulo frontend público da Loja (vitrine).
   Carregar depois de economizei-core.js e antes de carregarDados().

   Espelha o padrão de economizei-pedidos.js:
   - IIFE com namespace fechado (Economizei.Loja)
   - _abrirModalLocal / _fecharModalLocal (criarModalAcessivel por modal)
   - Cards.registrarModulo('loja', ...)
   - await Cards.dadosProntos dentro de abrirModal
   - Trava de duplo clique + validação de estoque + baixa
   - EU.confirmar() no lugar de confirm() nativo
   ============================================================ */
Economizei.Loja = (function () {
  'use strict';

  if (!window.Economizei) window.Economizei = {};
  if (!Economizei.Cards) { console.error('[loja] Core não carregado.'); return {}; }

  // CSS do módulo (continua sendo modulos/loja.css)
  (function () {
  ...
  /* ===== helpers de modal (uma instância por modal) ===== */
  ...
  // Compatibilidade com o CSV de outras páginas — registra no mesmo módulo
  ['loja online', 'ecommerce', 'e-commerce'].forEach(function (estilo) {
    Cards.registrarModulo(estilo, {
      label: 'Ver produtos',
      ariaLabel: 'Ver produtos da loja',
      icone: 'fa-solid fa-store',
      onClick: function (idx) { return 'Economizei.Loja.abrirModal(' + idx + ')'; }
  ...
  function obterGaleria(p) {
    ...
    (p && Array.isArray(p.variacoes) ? p.variacoes : []).forEach(function (v) {
      obterImagensVariacao(v).forEach(function (u) { add(u, v); });
    });
    return g;
  }
  ...
  function assinaturaAtributos(a) {
    if (!a || typeof a !== 'object') return '';
    return Object.keys(a).sort().map(function (k) { return k + '=' + String(a[k]); }).join('|');
  }
  function obterIdsVariacao(v) {
    if (!v) return [];
    var vals = [];
    ['variacaoId', 'sku', 'id'].forEach(function (c) {
      if (v[c] !== undefined && v[c] !== null && String(v[c]) !== '') vals.push(String(v[c]));
    });
  ...
  function faixaAplicada(p, preco, q) {
    ...
    if (p.quantidadeModo === 'a_partir') {
      p.faixas.forEach(function (f) {
        ...
        if (qf <= 0 || q < qf) return;
        var total = pf * q;
        if (total < sub && (!melhor || pf < melhor.preco || (pf === melhor.preco && qf > melhor.quantidade)))
          melhor = { quantidade: qf, preco: pf, pacotes: 0, restante: 0, subtotalPromocional: total };
      });
      return melhor;
    }
    p.faixas.forEach(function (f) {
      ...
      if (total < sub && (!melhor || total < melhor.subtotalPromocional))
        melhor = { quantidade: qf, preco: pf, pacotes: pac, restante: rest, subtotalPromocional: total };
    });
    return melhor;
  }
  ...
  function subtotalPromocao(p, preco, q) {
    ...
    if (p.tipo === 'percentual') {
      var pc = Math.min(100, Math.max(0, parseFloat(p.valor) || 0));
      return sub * (1 - pc / 100);
    }
    if (p.tipo === 'quantidade') {
      var f = faixaAplicada(p, preco, q);
      return f ? f.subtotalPromocional : sub;
    }
    return sub;
  }
  function resumoCondicao(p, f, preco) {
    ...
    if (p.tipo === 'quantidade') {
      var primeira = Array.isArray(p.faixas) ? p.faixas.map(function (x) {
        return { quantidade: parseInt(x.quantidade) || 0, preco: Math.max(0, parseFloat(x.preco) || 0) };
  ...
  function resumoPromocoes() {
    return carrinho.reduce(function (r, item) {
      ...
      if (o.promocao) r.promocoesAplicadas.push({
        id: o.promocao.id || null, nome: o.promocao.nome || 'Promoção',
        itemId: item.id, variacaoId: item.variacaoId || null, desconto: o.desconto
      });
      return r;
    }, { subtotalOriginal: 0, subtotalPromocional: 0, descontoPromocoes: 0, promocoesAplicadas: [] });
  }
  function obterOfertaProduto(p) {
    var cand = promocoesCache.filter(function (x) {
      return promocaoValida(x) && Array.isArray(x.produtoIds) &&
        x.produtoIds.some(function (id) { return String(id) === String(p.id); });
    });
    if (!cand.length) return null;
    var base = p.tipo === 'variavel' && Array.isArray(p.variacoes) && p.variacoes.length
      ? Math.min.apply(null, p.variacoes.map(function (v) { return parseFloat(v.preco) || 0; }))
    ...
    if (prom.aplicacao === 'especificas')
      return { promocao: prom, precoOriginal: base, precoPromocional: base, texto: 'Oferta em algumas variações', apenasAlgumas: true };
    ...
    var pf = Array.isArray(prom.faixas) ? prom.faixas.map(function (x) {
      return { quantidade: parseInt(x.quantidade) || 0, preco: Math.max(0, parseFloat(x.preco) || 0) };
    }).filter(function (x) { return x.quantidade > 0; }).sort(function (a, b) { return a.quantidade - b.quantidade; })[0] : null;
    var tq = pf ? (prom.quantidadeModo === 'a_partir'
      ? 'A partir de ' + pf.quantidade + ' un.: ' + formatarValor(pf.preco) + '/peça'
      : 'A cada ' + pf.quantidade + ' un.: ' + formatarValor(pf.preco) + '/grupo') : 'Oferta por quantidade';
  ...
  function atualizarCarrinho() {
    ...
    if (carrinho.length === 0) {
      cont.innerHTML = '<div class="loja-estado loja-carrinho-vazio" role="status"><span class="loja-estado-icone" aria-hidden="true">🛒</span><strong>Seu carrinho está vazio</strong><p>Explore os produtos e adicione seus favoritos.</p></div>';
      recalcularTotal(); atualizarBadge(); return;
    }
    var sub = 0;
    cont.innerHTML = carrinho.map(function (it) {
      sub += it.preco * it.quantidade;
      ...
      return '<div class="item-carrinho">' +
        '<img src="' + (it.imagem || 'https://via.placeholder.com/40') + '" class="item-carrinho-imagem" alt="" onerror="this.style.display=\'none\'">' +
        '<div style="flex:1"><strong>' + Core.sanitize(it.nome) + '</strong><br>' + ph + '</div>' +
        '<input type="number" min="1" value="' + it.quantidade + '" class="qtd-item" onchange="Economizei.Loja.alterarQuantidade(\'' + Core.jsEscape(it.id) + '\', this.value, \'' + Core.jsEscape(it.variacaoId || '') + '\')" aria-label="Quantidade">' +
        '<button class="btn-pequeno" onclick="Economizei.Loja.removerDoCarrinho(\'' + Core.jsEscape(it.id) + '\', \'' + Core.jsEscape(it.variacaoId || '') + '\')" aria-label="Remover">✕</button>' +
        '</div>';
    }).join('');
  ...
  function recalcularTotal() {
    ...
    var freteEl = document.getElementById('selectFreteLoja');
    var frete = parseFloat(freteEl && freteEl.value) || 0;
  ...
  function atualizarBadge() {
    ...
    if (total > 0) {
      ...
      b.setAttribute('aria-label', total + ' itens no carrinho');
  ...
  function adicionarAoCarrinho(prodId) {
    ...
    var qEl = document.getElementById('qtd_simples_' + prodId);
    var qt = qEl ? parseInt(qEl.value) : 1; if (isNaN(qt) || qt < 1) qt = 1;
    var ex = carrinho.find(function (i) { return i.id === prodId && !i.variacaoId; });
    if (ex) ex.quantidade += qt;
    else carrinho.push({
      id: p.id, nome: p.nome, preco: parseFloat(p.preco) || 0,
      quantidade: qt, imagem: p.imagem || null,
      variacaoId: null, estoque: p.estoque
    });
    atualizarCarrinho();
    UI.mostrarToast('Produto adicionado ao carrinho', 'sucesso');
  }
  function removerDoCarrinho(id, vId) {
    var item = carrinho.find(function (i) { return vId ? i.id === id && i.variacaoId === vId : i.id === id && !i.variacaoId; });
    if (!item) return;
    EU.confirmar('Remover "' + item.nome + '" do carrinho?', { titulo: 'Remover item', confirmar: 'Remover' }).then(function (ok) {
      if (!ok) return;
      carrinho = carrinho.filter(function (i) { return vId ? !(i.id === id && i.variacaoId === vId) : i.id !== id; });
      atualizarCarrinho();
    });
  }
  function alterarQuantidade(id, q, vId) {
    q = parseInt(q); if (isNaN(q) || q < 1) q = 1;
    var it = carrinho.find(function (i) { return vId ? i.id === id && i.variacaoId === vId : i.id === id && !i.variacaoId; });
    if (!it) return;
    var est = obterEstoqueNumerico(it.estoque);
    if (est !== null && q > est) {
      UI.mostrarToast('Estoque insuficiente. Disponível: ' + est, 'erro');
      atualizarCarrinho(); return;
    }
  ...
  function aplicarCupom() {
    ...
    Core.db.collection('lojistas').where('estabelecimentoId', '==', currentEstId).limit(1).get()
      .then(function (s) {
        if (s.empty) throw new Error('lojista_nao_encontrado');
        return Core.db.collection('lojistas').doc(s.docs[0].id).collection('cupons').where('codigo', '==', cod).where('ativo', '==', 'sim').limit(1).get();
      })
      .then(function (s) {
        ...
        var r = resumoPromocoes();
        var fEl = document.getElementById('selectFreteLoja');
        var fr = parseFloat(fEl && fEl.value) || 0;
        var d = calcularDescontoCupom(r.subtotalPromocional, fr);
        if (cupomAtual.minimoPedido && d <= 0) throw new Error('pedido_minimo');
        st.innerHTML = '<span style="color:#10b981;">✅ Cupom aplicado! Desconto de ' +
          (cupomAtual.tipo === 'percentual' ? cupomAtual.valor + '%' : 'R$ ' + (parseFloat(cupomAtual.valor) || 0).toFixed(2)) + '</span>';
        recalcularTotal();
      })
      .catch(function (err) {
        var min = cupomAtual && cupomAtual.minimoPedido;
        cupomAtual = null;
        var m = err.message === 'pedido_minimo'
          ? '❌ Pedido mínimo: R$ ' + (parseFloat(min) || 0).toFixed(2)
          : (err.message === 'cupom_invalido' ? '❌ Cupom inválido ou expirado' : '❌ Erro ao validar cupom');
  ...
  function toggleTroco() {
    ...
    if (ti) {
      ti.disabled = !din || (cb && cb.checked);
      if (cb && cb.checked) ti.value = '';
    }
  }
  ...
  function validarEstoqueAntesFinalizar() {
    ...
    carrinho.forEach(function (item) {
      var chave = item.id + '::' + (item.variacaoId || '');
      if (!totais[chave]) totais[chave] = { id: item.id, variacaoId: item.variacaoId || null, quantidade: 0, nome: item.nome };
      totais[chave].quantidade += parseInt(item.quantidade, 10) || 0;
    });
    return Promise.all(Object.keys(totais).map(function (chave) {
      var t = totais[chave];
      return Core.db.collection('lojistas').doc(currentLojistaId).collection('cardapio').doc(t.id).get().then(function (snap) {
        if (!snap.exists) { var e = new Error('Produto não encontrado no cardápio.'); e.estoque = true; throw e; }
        var dado = snap.data() || {};
        if (t.variacaoId && dado.tipo === 'variavel' && Array.isArray(dado.variacoes)) {
          var idsItem = [String(t.variacaoId)];
          // tenta reconstruir a assinatura pelo item do carrinho
          var itemCarrinho = carrinho.find(function (i) { return i.id === t.id && (i.variacaoId || '') === String(t.variacaoId || ''); });
          if (itemCarrinho && itemCarrinho.atributos) idsItem = idsItem.concat(obterIdsVariacao(itemCarrinho));
          var achou = false;
          dado.variacoes.forEach(function (v) {
            if (achou) return;
            var idsVar = obterIdsVariacao(v);
            var bate = idsItem.some(function (idItem) { return idsVar.indexOf(idItem) !== -1; });
            if (!bate) return;
            achou = true;
            var est = obterEstoqueNumerico(v.estoque);
            if (est !== null && t.quantidade > est) {
              var e2 = new Error('Estoque insuficiente para ' + t.nome + '. Disponível: ' + est); e2.estoque = true; throw e2;
            }
          });
          if (!achou) { var e3 = new Error('Variação não encontrada para ' + t.nome + '.'); e3.estoque = true; throw e3; }
          return;
        }
        var est = obterEstoqueNumerico(dado.estoque);
        if (est !== null && t.quantidade > est) {
          var e4 = new Error('Estoque insuficiente para ' + t.nome + '. Disponível: ' + est); e4.estoque = true; throw e4;
        }
      });
    }));
  }
  ...
  function baixarEstoque(carrinhoSnap) {
    var updates = carrinhoSnap.map(function (item) {
      if (estoqueSemLimite(item.estoque)) return Promise.resolve();
      return Core.db.collection('lojistas').doc(currentLojistaId).collection('cardapio').doc(item.id).get()
        .then(function (doc) {
          if (!doc.exists) return;
          var data = doc.data();
          if (item.variacaoId && data.tipo === 'variavel' && Array.isArray(data.variacoes)) {
            var idsItem = [String(item.variacaoId)].concat(item.atributos ? obterIdsVariacao(item) : []);
            var alterou = false;
            var variacoes = data.variacoes.map(function (v) {
              ...
              alterou = true;
              var est = obterEstoqueNumerico(v.estoque);
              if (est === null) return v;
              var novo = Math.max(0, est - item.quantidade);
              return Object.assign({}, v, { estoque: String(novo) });
            });
            if (alterou) return doc.ref.update({ variacoes: variacoes });
            return;
          }
          var est = obterEstoqueNumerico(data.estoque);
          if (est === null) return;
          var novo = Math.max(0, est - item.quantidade);
          return doc.ref.update({ estoque: String(novo) });
        });
    });
  ...
  function finalizarPedido() {
    ...
    if (fObrig && !fSel) {
      UI.mostrarToast('Selecione o frete antes de confirmar.', 'erro');
      selF.focus(); selF.setAttribute('aria-invalid', 'true'); return;
    }
    ...
    if (!currentLojistaId) { UI.mostrarToast('Erro: lojista não identificado. Tente reabrir o modal.', 'erro'); return; }
    ...
    var itens = carrinho.map(function (i) {
      ...
      return {
        ...
        precoUnitario: o.precoUnitarioPromocional, precoUnitarioOriginal: i.preco,
        variacaoId: i.variacaoId || null, atributos: i.atributos || null,
        promocao: o.promocao ? o.promocao.nome : null
      };
    });
    var cod = Math.random().toString(36).substring(2, 8).toUpperCase();
    var pedido = {
      estabelecimentoId: currentEstId,
      estabelecimentoNome: document.getElementById('modalLojaTitulo').textContent,
    ...
    validarEstoqueAntesFinalizar()
      .then(function () { return salvarPedidoComCupom(pedido); })
      .then(function () {
        ...
        atualizarCarrinho();
        baixarEstoque(carrinhoParaEstoque).catch(function (err) {
          console.warn('Baixa de estoque não aplicada (pedido já confirmado normalmente):', err.message);
        });
      })
      .catch(function (err) {
        if (err && err.message === 'cupom_limite') {
          cupomAtual = null; cupomDesconto = 0; recalcularTotal();
          UI.mostrarToast('O limite de usos deste cupom já foi atingido.', 'erro');
          return;
        }
        UI.mostrarToast(err && err.estoque ? err.message : 'Erro ao finalizar pedido: ' + err.message, 'erro');
      })
      .finally(function () {
  ...
  /* ============================================================
     CONSULTA / HISTÓRICO / COMPROVANTE
     ============================================================ */
  function consultarPedido() {
    ...
    Core.db.collection('pedidos').where('codigoCurto', '==', cod).limit(1).get().then(function (s) {
      ...
      var p = s.docs[0].data();
      var map = {
        pendente:     { l: 'Aguardando Loja',  i: '⏳', c: '#f59e0b' },
        confirmado:   { l: 'Confirmado',       i: '✅', c: '#10b981' },
        em_preparo:   { l: 'Em Preparação',    i: '📦', c: '#6366f1' },
        saiu_entrega: { l: 'Saiu para Entrega', i: '🚚', c: '#3b82f6' },
        concluido:    { l: 'Entregue',          i: '✅', c: '#10b981' },
        cancelado:    { l: 'Cancelado',         i: '❌', c: '#ef4444' }
      };
  ...
  function verComprovanteHistorico(id) {
    ...
    gerarComprovante(p, p.codigoCurto || id.slice(0, 6).toUpperCase());
  }
  function verComprovante(cod) {
    Core.db.collection('pedidos').where('codigoCurto', '==', cod).limit(1).get().then(function (s) {
      ...
      .then(function (s) {
        ...
        s.forEach(function (d) {
          ...
          h += '<div style="border:1px solid var(--gray-200);border-radius:.75rem;padding:.75rem;margin-bottom:.5rem;background:#fff;">' +
            ...
            '<div><strong>#' + Core.sanitize(cod) + '</strong> <span style="background:' +
            (p.status === 'pendente' ? '#fff7ed' : p.status === 'concluido' ? '#ecfdf5' : '#f1f5f9') +
            ';padding:.2rem .5rem;border-radius:1rem;font-size:.7rem;">' + Core.sanitize(p.status || 'pendente') + '</span></div>' +
            '<div>' + (p.criadoEm ? new Date(p.criadoEm.toDate()).toLocaleString() : '---') + '</div></div>' +
            '<div><strong>Total:</strong> R$ ' + (p.total || 0).toFixed(2) + '</div>' +
            '<button class="btn-acao btn-qrcode" style="margin-top:.5rem;" onclick="Economizei.Loja.verComprovanteHistorico(\'' + d.id + '\')">🖨️ Ver comprovante</button>' +
            '</div>';
        });
        c.innerHTML = h;
      });
  ...
  function renderizarProdutos(produtos) {
    if (!produtos || !produtos.length) {
      return '<div class="loja-estado loja-estado-vazio" role="status"><span class="loja-estado-icone" aria-hidden="true">⌕</span><strong>Vitrine vazia por enquanto</strong><p>Esta loja ainda não publicou produtos.</p></div>';
    }
    var cats = {};
    ...
    var h = '';
    for (var c in cats) {
      h += '<div class="categoria-group">' +
        '<button type="button" class="categoria-titulo-modal" aria-expanded="true">' + Core.sanitize(c) + '</button>' +
  ...
  function gerarHTMLProduto(p) {
    ...
    if (tipo === 'variavel') {
      ...
      var tot = p.variacoes.reduce(function (a, v) {
        var e = obterEstoqueNumerico(v.estoque); return a + (e === null ? 0 : e);
      }, 0);
      esgotado = !ilim && tot <= 0;
    } else {
      var e = obterEstoqueNumerico(p.estoque);
      if (e !== null) esgotado = e <= 0;
    }
    var imgHtml = imgPrinc
      ? '<img src="' + imgPrinc + '" loading="lazy" alt="' + Core.sanitize(p.nome) + '" onclick="Economizei.Loja.abrirProdutoPeloCard(\'' + Core.jsEscape(p.id) + '\')" style="cursor:pointer;">'
      : '<div class="produto-sem-imagem" aria-label="Sem imagem" onclick="Economizei.Loja.abrirProdutoPeloCard(\'' + Core.jsEscape(p.id) + '\')" style="cursor:pointer;"><i class="fas fa-image" aria-hidden="true"></i></div>';
    var precoTxt = '';
    if (temV) {
    ...
    if (tipo === 'variavel') {
      ...
      estH = il2
        ? '<div style="font-size:.65rem;color:#64748b;">Estoque: Ilimitado</div>'
        : '<div style="font-size:.65rem;color:' + (t2 <= 5 ? '#dc3545' : '#64748b') + ';">Estoque total: ' + t2 + '</div>';
    } else {
      var eg = obterEstoqueNumerico(p.estoque);
      estH = eg === null
        ? '<div style="font-size:.65rem;color:#64748b;">Estoque: Ilimitado</div>'
        : '<div style="font-size:.65rem;color:' + (eg <= 5 ? '#dc3545' : '#64748b') + ';">Estoque: ' + eg + '</div>';
    }
    var btn = '';
    if (temV) {
      btn = '<button class="btn-escolher" data-prod-id="' + p.id + '" data-tipo="variavel" ' + (esgotado ? 'disabled' : '') + '><i class="fas fa-cart-plus" aria-hidden="true"></i> ' + (esgotado ? 'Indisponível' : 'Escolher') + '</button>';
    } else {
      btn = '<div class="produto-quantidade-simples">' +
        '<button type="button" class="qtd-btn-simples" data-prod-id="' + p.id + '" data-delta="-1" aria-label="Diminuir">−</button>' +
        '<input type="number" id="qtd_simples_' + p.id + '" value="1" min="1" style="width:3rem;text-align:center;" aria-label="Quantidade">' +
        '<button type="button" class="qtd-btn-simples" data-prod-id="' + p.id + '" data-delta="1" aria-label="Aumentar">+</button>' +
        '</div>' +
        '<button class="btn-adicionar-simples" data-prod-id="' + p.id + '" data-preco="' + (parseFloat(p.preco) || 0) + '" ' + (esgotado ? 'disabled' : '') + '><i class="fas fa-cart-plus" aria-hidden="true"></i> ' + (esgotado ? 'Indisponível' : 'Adicionar') + '</button>';
    }
    return '<div class="produto-card" data-prod-id="' + p.id + '">' + imgHtml +
      '<div class="card-content-produto">' +
      '<div class="produto-nome">' + Core.sanitize(p.nome) + '</div>' +
      '<div class="produto-preco">' + (of && !of.apenasAlgumas && of.promocao.tipo !== 'quantidade'
        ? '<s style="color:#94a3b8;font-size:.72rem;margin-right:.25rem;">' + precoTxt + '</s> R$ ' + of.precoPromocional.toFixed(2)
        : precoTxt) + '</div>' +
      promH + estH +
      '</div>' + btn + '</div>';
  }
  ...
  function abrirProdutoPeloCard(prodId) {
    var p = produtosCache.find(function (x) { return x.id === prodId; });
    if (!p) return;
    if (p.tipo === 'variavel' && p.variacoes && p.variacoes.length) abrirModalVariacoes(p);
    else abrirImagemFull(p);
  }
  ...
  function abrirImagemFull(p) {
    ...
    m.setAttribute('aria-label', 'Imagem de ' + p.nome);
    ...
    function atualizar() {
      ...
      var estH = en === null
        ? '<div style="font-size:.8rem;color:#cbd5e1;">Estoque: Ilimitado</div>'
        : '<div style="font-size:.8rem;color:#cbd5e1;">Estoque: ' + en + '</div>';
      m.innerHTML =
        '<div style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;width:100%;height:100%;background:#000;position:relative;">' +
        ...
        (imgs.length > 1
          ? '<button type="button" id="imgAnt" class="imagem-navegacao" style="position:absolute;top:50%;left:.75rem;transform:translateY(-50%);width:42px;height:58px;border-radius:999px;background:rgba(15,23,42,.72);color:#fff;font-size:42px;border:1px solid rgba(255,255,255,.35);cursor:pointer;">‹</button>' +
            '<button type="button" id="imgSeg" class="imagem-navegacao" style="position:absolute;top:50%;right:.75rem;transform:translateY(-50%);width:42px;height:58px;border-radius:999px;background:rgba(15,23,42,.72);color:#fff;font-size:42px;border:1px solid rgba(255,255,255,.35);cursor:pointer;">›</button>'
          : '') +
        ...
        (imgs.length > 1
          ? '<div style="display:flex;gap:.5rem;justify-content:center;margin-top:.5rem;flex-wrap:wrap;">' +
            imgs.map(function (u, i) {
              return '<img src="' + u + '" data-idx="' + i + '" alt="Miniatura ' + (i + 1) + '" style="width:40px;height:40px;object-fit:cover;border-radius:.5rem;cursor:pointer;border:2px solid ' + (i === cur ? '#0a66c2' : 'transparent') + ';">';
            }).join('') + '</div>'
          : '') +
        '</div>' +
        ...
        '<button type="button" id="mQ" style="background:#333;color:#fff;border:none;border-radius:50%;width:26px;height:26px;cursor:pointer;">−</button>' +
        '<input type="number" id="qtdImg" value="1" min="1" style="width:60px;text-align:center;border:1px solid #444;border-radius:2rem;background:#222;color:#fff;" aria-label="Quantidade">' +
        '<button type="button" id="pQ" style="background:#333;color:#fff;border:none;border-radius:50%;width:26px;height:26px;cursor:pointer;">+</button>' +
        '</div>' +
        '<button class="btn-pedido-cta" id="addImg">Adicionar ao carrinho</button>' +
        '</div></div>';
      m.querySelectorAll('[data-idx]').forEach(function (mini) {
        mini.addEventListener('click', function () { cur = parseInt(mini.dataset.idx); atualizar(); });
      });
      ...
      m.querySelector('#addImg').addEventListener('click', function () {
        ...
        if (p.tipo === 'variavel' && p.variacoes && p.variacoes.length) {
          _fecharModalLocal(m); abrirModalVariacoes(p); return;
        }
        ...
        if (ex) ex.quantidade += q;
          ...
          id: p.id, nome: p.nome, preco: pBase, quantidade: q,
          imagem: p.imagem || null, variacaoId: null, estoque: p.estoque
        });
        atualizarCarrinho();
        UI.mostrarToast('Produto adicionado ao carrinho', 'sucesso');
        _fecharModalLocal(m);
  ...
  function abrirModalVariacoes(p) {
    ...
    for (var i = 0; i < vs.length; i++) {
      var e = obterEstoqueNumerico(vs[i].estoque);
      if (e === null || e > 0) { prim = vs[i]; break; }
    }
    ...
    m.setAttribute('role', 'dialog');
    m.setAttribute('aria-modal', 'true');
    m.setAttribute('aria-label', 'Escolher variação de ' + p.nome);
    var imgPrim = obterImagensVariacao(prim)[0] || obterImagemPrincipal(p) || 'https://via.placeholder.com/300';
    m.innerHTML =
      '<div class="modal-conteudo modal-variacao-full">' +
    ...
    document.body.appendChild(m);
    _abrirModalLocal(m);
    var aC = document.getElementById('attrC'), vI = document.getElementById('vImg'), vN = document.getElementById('vInfo');
    var qi = document.getElementById('vQtd'), bA = document.getElementById('addVar');
    bA.disabled = false;
    var varSel = prim;
    function atualizarVar() {
      ...
      for (var i = 0; i < vs.length; i++) {
        var v = vs[i];
        var ok = true;
        for (var a in sel) { if (v.atributos[a] !== sel[a]) { ok = false; break; } }
        if (ok) { enc = v; break; }
      }
      if (enc) {
        varSel = enc;
        ...
        if (es === null) qi.removeAttribute('max');
        else { qi.max = String(es); if (parseInt(qi.value) > es && es > 0) qi.value = es; }
        var q = Math.max(1, parseInt(qi.value) || 1);
        document.getElementById('vTotal').textContent = 'R$ ' + (pr * q).toFixed(2);
        bA.disabled = es !== null && es <= 0;
      } else {
        vN.innerHTML = 'Combinação não disponível.';
        document.getElementById('vTotal').textContent = 'Indisponível';
        vI.src = obterImagemPrincipal(p);
        bA.disabled = true;
      }
    }
    aC.innerHTML = '';
    ...
    vI.addEventListener('click', function () {
      ...
      if (enc) {
        cl.__precoSelecionado = parseFloat(enc.preco) || 0;
        cl.__estoqueSelecionado = enc.estoque;
        cl.__variacaoSelecionada = JSON.parse(JSON.stringify(enc));
      }
      abrirImagemFull(cl);
    });
    document.getElementById('vMenos').addEventListener('click', function () {
      var v = parseInt(qi.value) || 1; if (v > 1) { qi.value = v - 1; atualizarVar(); }
    });
    document.getElementById('vMais').addEventListener('click', function () {
      var v = parseInt(qi.value) || 1;
      var mx = qi.max ? parseInt(qi.max, 10) : Infinity;
      if (!Number.isFinite(mx) || v < mx) { qi.value = v + 1; atualizarVar(); }
    });
    ...
    bA.addEventListener('click', function () {
      ...
      if (ex) ex.quantidade += q;
        ...
        id: p.id, nome: nomeComp,
        preco: parseFloat(varSel.preco) || 0,
        quantidade: q, imagem: varSel.imagem || p.imagem || null,
        variacaoId: vId, atributos: JSON.parse(JSON.stringify(sel)),
        estoque: varSel.estoque
      });
      atualizarCarrinho();
      UI.mostrarToast('Produto adicionado', 'sucesso');
      _fecharModalLocal(m);
    });
    atualizarVar();
  }
  ...
  function atualizarLogoUI(data) {
    ...
    if (!url) { box.textContent = gerarIniciais(document.getElementById('modalLojaTitulo').textContent); return; }
    ...
    img.onerror = function () {
      box.removeAttribute('data-logo-url');
      box.textContent = gerarIniciais(document.getElementById('modalLojaTitulo').textContent);
    };
    box.appendChild(img);
  }
  function atualizarStatusUI(status, msg) {
    ...
    if (btn) {
      btn.disabled = bloq;
      btn.style.opacity = bloq ? '.5' : '1';
      btn.style.pointerEvents = bloq ? 'none' : 'auto';
    }
  }
  ...
  async function abrirModal(idx) {
    ...
    var dados = Cards.dadosProcessados;
    var est = dados[idx];
    if (!est) { console.error('[loja] Estabelecimento não encontrado:', idx); UI.mostrarToast('Estabelecimento não encontrado.'); return; }
    var nomeEstab = est[COLUNAS.NOME];
    var logoEstab = est[COLUNAS.IMAGEM] || '';
    var estId = est[COLUNAS.ID_UNICO];
    currentEstId = estId;
    currentLojistaId = null;
    carrinho = []; cupomDesconto = 0; cupomAtual = null;
    promocoesCache = []; produtosCache = []; fretesCache = [];
    statusLojaAtual = 'aberta';
    enviandoPedido = false;

    // fecha instância anterior, se houver
    var antigo = document.getElementById('modalLoja');
    if (antigo) { _pararListeners(); _fecharModalLocal(antigo); }

    var logoHtml = logoEstab
      ? '<img src="' + Core.sanitize(logoEstab) + '" alt="Logo" loading="eager" referrerpolicy="no-referrer">'
      : '<span aria-hidden="true">🛍️</span>';
    var logado = !!Core.getCurrentUser();
    var html = '<div class="modal-overlay loja-padronizada" id="modalLoja" role="dialog" aria-modal="true" aria-labelledby="modalLojaTitulo">' +
      ...
      '<div class="modal-estabelecimento-logo">' + logoHtml + '</div>' +
      '<div class="modal-estabelecimento-meta">' +
      '<h3 id="modalLojaTitulo">' + Core.sanitize(nomeEstab) + '</h3>' +
      '<span id="statusLojaBadgeLoja" class="modal-estabelecimento-status status-aberta">Aceitando pedidos</span>' +
      '</div></div>' +
      '<button class="modal-close-btn" data-fechar-modal aria-label="Fechar loja">×</button>' +
      '</div>' +
      ...
      (logado ? '<button class="modal-tab" data-tab="historico">📋 Histórico</button>' : '') +
      '</div>' +
      ...
      '<label class="sem-troco-label"><input type="checkbox" id="semTrocoCheckbox"> Não preciso de troco</label>' +
      '</div>' +
      '<div class="cliente-info">' +
      '<div class="form-row">' +
      '<input type="text" id="clienteNomeLoja" class="input-pedido" placeholder="Seu nome*" value="' + Core.sanitize(Core.getUserDisplayName() || '') + '" aria-label="Seu nome" required>' +
      ...
      '</div>' +
      '<div id="resultadoRastreio" style="margin-top:1rem;" aria-live="polite"></div>' +
      '</div>' +
      (logado ? '<div id="tabHistorico" class="modal-tab-content"><div id="listaHistoricoLoja"></div></div>' : '') +
      '</div></div></div>';
    document.body.insertAdjacentHTML('beforeend', html);
    var modalEl = document.getElementById('modalLoja');
    ...
    // delegação de click nos produtos
    var pc = document.getElementById('produtosContainer');
    if (pc) pc.addEventListener('click', function (e) {
      var cat = e.target.closest('.categoria-titulo-modal');
      if (cat) {
        var grupo = cat.nextElementSibling;
        var expandir = cat.getAttribute('aria-expanded') !== 'true';
        if (grupo) grupo.style.display = expandir ? 'grid' : 'none';
        cat.setAttribute('aria-expanded', String(expandir));
        cat.classList.toggle('recolhida', !expandir);
        return;
      }
      var qb = e.target.closest('.qtd-btn-simples');
      if (qb) {
        var pid = qb.dataset.prodId, d = parseInt(qb.dataset.delta);
        var inp = document.getElementById('qtd_simples_' + pid);
        if (inp) inp.value = Math.max(1, (parseInt(inp.value) || 1) + d);
        return;
      }
      var eb = e.target.closest('.btn-escolher');
      if (eb) {
        var p2 = produtosCache.find(function (x) { return x.id === eb.dataset.prodId; });
        if (p2 && p2.tipo === 'variavel') abrirModalVariacoes(p2);
        return;
      }
    ...
    // carrega lojista + listeners
    Core.db.collection('lojistas').where('estabelecimentoId', '==', estId).limit(1).get()
      .then(function (snap) {
        if (snap.empty) {
          var c = document.getElementById('produtosContainer');
          if (c) c.innerHTML = '<div class="loja-estado loja-estado-erro" role="alert"><span class="loja-estado-icone" aria-hidden="true">!</span><strong>Estabelecimento não configurado para loja</strong><p>Entre em contato com o lojista para mais informações.</p></div>';
          return;
        }
        var ld = snap.docs[0];
        ...
        atualizarStatusUI(statusSeguro(data.statusLoja), data.statusMessage || '');
        unsubscribeStatusLoja = Core.db.collection('lojistas').doc(currentLojistaId).onSnapshot(function (d) {
          if (!d.exists) return;
          var dt = d.data();
          atualizarLogoUI(dt);
          atualizarStatusUI(statusSeguro(dt.statusLoja), dt.statusMessage || '');
        });
        unsubscribeCardapio = Core.db.collection('lojistas').doc(currentLojistaId).collection('cardapio').onSnapshot(function (s) {
          ...
          s.forEach(function (d) {
            ...
            var vs = Array.isArray(dd.variacoes) ? dd.variacoes.map(function (v) {
              ...
              c.imagens = iv;
              c.imagem = c.imagem || iv[0] || '';
              return c;
            }) : [];
            var ip = dd.imagem || imgs[0] || null;
            var fb = false;
            if (!ip) for (var i = 0; i < vs.length; i++) {
              var iv2 = obterImagensVariacao(vs[i]);
              if (iv2.length) { ip = iv2[0]; fb = true; break; }
            }
            var p = {
              id: d.id, nome: dd.nome, preco: parseFloat(dd.preco) || 0,
              imagem: ip, __imagemFallbackVariacao: fb,
              descricao: dd.descricao || '',
              estoque: dd.estoque !== undefined && dd.estoque !== '' ? parseInt(dd.estoque) : null,
              categoria: dd.categoria || 'Geral',
              tipo: dd.tipo || 'simples',
              atributos: dd.atributos || [],
              variacoes: vs, imagens: imgs
            };
            if (p.tipo === 'variavel' && p.variacoes.length) {
              p.variacoes = p.variacoes.map(function (v) {
                if (!v.sku) {
                  var sp = [];
                  for (var a in v.atributos) sp.push(v.atributos[a].substring(0, 3).toUpperCase());
                  v.sku = p.id.substring(0, 4) + '-' + sp.join('-');
                }
                return v;
              });
            }
          ...
          var c = document.getElementById('produtosContainer');
          if (c) c.innerHTML = '<div class="loja-estado loja-estado-erro" role="alert"><span class="loja-estado-icone" aria-hidden="true">!</span><strong>Não foi possível carregar os produtos</strong><p>Verifique sua conexão e tente novamente.</p></div>';
          console.error('[loja] erro cardapio:', err);
        });
        unsubscribeFretes = Core.db.collection('lojistas').doc(currentLojistaId).collection('fretes').onSnapshot(function (s) {
          var frs = [];
          s.forEach(function (d) {
            var dd = d.data();
            if (dd.ativo === 'nao') return;
            frs.push({ localidade: dd.localidade, taxa: parseFloat(dd.taxa) || 0 });
          });
          fretesCache = frs;
          var sel = document.getElementById('selectFreteLoja');
          if (sel) {
            sel.innerHTML = '<option value="">' + (frs.length ? 'Selecione o frete' : 'Nenhuma opção de frete cadastrada') + '</option>' +
              frs.map(function (f) { return '<option value="' + f.taxa + '">' + Core.sanitize(f.localidade) + ' - R$ ' + f.taxa.toFixed(2) + '</option>'; }).join('');
            if (sel.options[0]) sel.options[0].disabled = frs.length > 0;
      ...
      .catch(function (err) {
        var c = document.getElementById('produtosContainer');
        if (c) c.innerHTML = '<div class="loja-estado loja-estado-erro" role="alert"><span class="loja-estado-icone" aria-hidden="true">!</span><strong>Não foi possível carregar a loja</strong><p>' + Core.sanitize(err.message) + '</p></div>';
      });
  }
  ...
  var API = {
    ...
    verComprovante: verComprovante,
    verComprovanteHistorico: verComprovanteHistorico
  };
  window.Economizei = window.Economizei || {};
  window.Economizei.Loja = API;
  return API;
})();
