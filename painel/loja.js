(function () {
    ...
    function adicionarAtributo() {
        ...
        EU.mostrarToast('Atributo adicionado!', 'sucesso');
    }
    ...
    function detalhesVariacaoHtml(v, idx, tipoAgrupamento) {
        ...
        return atributosHtml +
            ...
            '<div class="variacao-imagens-lista">' + renderMiniaturasVariacao(imagens, idx, 'novo') + '</div>' +
    ...
    // ===== PEDIDOS (mesma lógica do Pedidos) =====
    ...
    async function toggleAtivoProduto(id, ativo) {
        try {
            ...
            EU.mostrarToast(ativo ? 'Produto ativado!' : 'Produto marcado como inativo.', 'sucesso');
    ...
    async function excluirItemCardapio(id) {
        ...
        EU.showLoading('Excluindo...');
        try { await db.collection('lojistas').doc(emailAtual).collection('cardapio').doc(id).delete(); await carregarCardapio(); EU.mostrarToast('Produto removido!', 'sucesso'); }
    ...
    function editarFreteModal(id, loc, taxa, ativo, isDuplicar) {
        ...
        document.getElementById('salvarFrete').onclick = async function () {
            ...
            if (!dados.localidade || !Number.isFinite(dados.taxa) || dados.taxa < 0) { EU.mostrarToast('Preencha uma localidade e uma taxa válida.', 'erro'); return; }
            try {
                var ref = db.collection('lojistas').doc(emailAtual).collection('fretes');
                if (isDuplicar) { await ref.add(dados); EU.mostrarToast('Taxa duplicada!', 'sucesso'); }
                else { await ref.doc(id).update(dados); EU.mostrarToast('Taxa atualizada!', 'sucesso'); }
    ...
    async function salvarCupomLoja() {
        ...
        try {
            ...
            if (cupomEmEdicao && !cupomEmEdicao.isDuplicar) {
                ...
                EU.mostrarToast('Cupom atualizado!', 'sucesso');
            } else {
                await ref.add(Object.assign({}, dados, { usosTotal: 0, usosPorCliente: {} }));
                EU.mostrarToast(cupomEmEdicao && cupomEmEdicao.isDuplicar ? 'Cupom duplicado!' : 'Cupom criado!', 'sucesso');
            }
    ...
    async function salvarPromocaoLoja() {
        ...
        try {
            ...
            if (promocaoEmEdicao && !promocaoEmEdicao.isDuplicar) {
                ...
                EU.mostrarToast('Promoção atualizada!', 'sucesso');
            } else {
                dados.criadoEm = firebase.firestore.FieldValue.serverTimestamp();
                await ref.add(dados);
                EU.mostrarToast(promocaoEmEdicao && promocaoEmEdicao.isDuplicar ? 'Promoção duplicada!' : 'Promoção criada!', 'sucesso');
            }
    ...
    async function excluirPromocao(id) {
        ...
        try { await db.collection('lojistas').doc(emailAtual).collection('promocoes').doc(id).delete(); await carregarPromocoes(); EU.mostrarToast('Promoção removida!', 'sucesso'); }
        catch (e) { EU.mostrarToast(e.message, 'erro'); }
    }
