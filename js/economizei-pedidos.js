...
Economizei.Pedido = (function() {
    ...
    function abrirImagemProduto(prodId, visualizarImagem) { var produto = produtosCache.find(function(p) { return p.id === prodId; }); if (!produto) { UI.mostrarToast('Produto não encontrado.'); return; } if (produto.personalizavel === true && !visualizarImagem) { abrirModalCustomizacaoCompleta(produto); return; } abrirModalImagemFull(produto); }
    ...
    function abrirModalImagemFull(produto) {
        ...
        function atualizarModal() {
            ...
            modal.querySelector('#addImagem').addEventListener('click', function() { if (produto.personalizavel) { modal.remove(); abrirModalCustomizacaoCompleta(produto); return; } var qtd = parseInt(qtdInput.value) || 1; if (!validarEstoqueAdicao(produto, qtd)) return; var nomeCompleto = tamanhoSelecionado ? produto.nome + ' (' + tamanhoSelecionado + ')' : produto.nome; var existing = carrinho.find(function(i) { return i.id === produto.id && i.tamanho === tamanhoSelecionado; }); if (existing) existing.quantidade += qtd; else carrinho.push(Object.assign({}, produto, { id: produto.id, nome: nomeCompleto, preco: precoSelecionado, quantidade: qtd, tamanho: tamanhoSelecionado })); atualizarCarrinhoVisual(); recalcularTotal(); atualizarBadgeCarrinho(); UI.mostrarToast('Produto adicionado ao carrinho'); modal.remove(); });
        }
    ...
    function abrirModalExtrasSimples(produto) {
        ...
        modal.innerHTML = '<div class="modal-conteudo modal-extras-pontual" style="max-width:500px;"><div class="modal-header"><h3>' + Core.sanitize(produto.nome) + '</h3><button class="btn-modal-fechar" onclick="this.closest(\'.modal-overlay\').remove()" aria-label="Fechar">✕</button></div><div class="modal-body"><div style="margin-bottom:0.5rem;">Preço: R$ ' + precoBase.toFixed(2) + '</div><div><strong>Extras</strong></div><div id="extrasSimplesContainer" style="margin-top:0.5rem;">' + (selecaoExtras.length === 0 ? '<p style="text-align:center;color:var(--gray-500);">Nenhum extra disponível.</p>' : '') + '</div><div id="precoTotalSimples" class="preco-total-custom" style="margin-top:1rem;">Total: R$ ' + precoBase.toFixed(2) + '</div></div><div class="modal-footer"><button class="btn-pedido-cta" id="btnAddExtraSimples">Adicionar ao carrinho</button></div></div>';
        document.body.appendChild(modal); UI.trapFocus(modal);
