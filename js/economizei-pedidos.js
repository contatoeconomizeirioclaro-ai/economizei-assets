...
var enviandoPedido = false; // NOVO: trava contra duplo clique
    ...
    function abrirModalExtrasSimples(produto) {
        ...
        modal.innerHTML = '<div class="modal-conteudo modal-extras-responsivo"><div class="modal-header"><h3>' + Core.sanitize(produto.nome) + '</h3><button class="btn-modal-fechar" onclick="this.closest(\'.modal-overlay\').remove()" aria-label="Fechar">✕</button></div><div class="modal-body"><div style="margin-bottom:0.5rem;">Preço: R$ ' + precoBase.toFixed(2) + '</div><div><strong>Extras</strong></div><div id="extrasSimplesContainer" style="margin-top:0.5rem;">' + (selecaoExtras.length === 0 ? '<p style="text-align:center;color:var(--gray-500);">Nenhum extra disponível.</p>' : '') + '</div><div id="precoTotalSimples" class="preco-total-custom" style="margin-top:1rem;">Total: R$ ' + precoBase.toFixed(2) + '</div></div><div class="modal-footer"><button class="btn-pedido-cta" id="btnAddExtraSimples">Adicionar ao carrinho</button></div></div>';
        document.body.appendChild(modal); UI.trapFocus(modal);
