...
var enviandoPedido = false; // NOVO: trava contra duplo clique
    ...
    function garantirEstilosModaisPedidos() {
        ...
        style.textContent = `
:root{--pedido-blue:#0a66c2;--pedido-blue-dark:#084d94;--pedido-surface:#f8fafc;--pedido-border:#dbe4ee;--pedido-muted:#64748b;--pedido-success:#16a34a;--pedido-shadow:0 18px 50px rgba(15,23,42,.18)}
...
body.modo-app .modal-pedido-unificado .modal-footer,body.modo-app .modal-configuracao-produto .modal-footer,body.modo-app .modal-extras-responsivo .modal-footer{padding-bottom:calc(1rem + var(--reserva-barra-app,0px))!important}
...
.config-empty-state{margin:0;padding:1.1rem;text-align:center;color:#64748b;font-size:.8rem;border:1px dashed #cbd5e1;border-radius:.75rem;background:#f8fafc}.modal-footer-summary{display:flex;flex-direction:column;gap:.15rem;color:var(--pedido-muted);font-size:.75rem}
    ...
    function abrirModalImagemFull(produto) {
        ...
        function atualizarModal() {
            ...
            if (produto.personalizavel === true) {
                ...
                addImagemBtn.addEventListener('click', function() { var qtd = parseInt(qtdInput.value) || 1; if (produto.estoque !== null && produto.estoque !== undefined && produto.estoque !== '') { var estoqueDisponivel = parseInt(produto.estoque); if (!isNaN(estoqueDisponivel) && estoqueDisponivel < qtd) { UI.mostrarToast('Estoque insuficiente. Disponível: ' + estoqueDisponivel); return; } } var nomeCompleto = tamanhoSelecionado ? produto.nome + ' (' + tamanhoSelecionado + ')' : produto.nome; var existing = carrinho.find(function(i) { return i.id === produto.id && i.tamanho === tamanhoSelecionado; }); if (existing) existing.quantidade += qtd; else carrinho.push(Object.assign({}, produto, { id: produto.id, nome: nomeCompleto, preco: precoSelecionado, quantidade: qtd, tamanho: tamanhoSelecionado })); atualizarCarrinhoVisual(); recalcularTotal(); atualizarBadgeCarrinho(); UI.mostrarToast('Produto adicionado ao carrinho'); modal.remove(); });
            }
        }
