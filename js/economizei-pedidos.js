...
Economizei.Pedido = (function() {
    ...
    // Ajustes pontuais, sem alterar o layout geral da página.
    (function() {
        if (document.getElementById('economizei-pedidos-pontual-fixes')) return;
        var style = document.createElement('style');
        style.id = 'economizei-pedidos-pontual-fixes';
        style.textContent = '#semTrocoCheckbox{appearance:none!important;-webkit-appearance:none!important;width:1.05rem!important;height:1.05rem!important;min-width:1.05rem!important;margin:0!important;border:2px solid #94a3b8!important;border-radius:.25rem!important;background:#fff!important;display:inline-grid!important;place-content:center!important;cursor:pointer!important;vertical-align:middle!important}#semTrocoCheckbox:checked{background:#0a66c2!important;border-color:#0a66c2!important}#semTrocoCheckbox:checked::after{content:"✓";color:#fff;font-size:.72rem;font-weight:800;line-height:1}#semTrocoCheckbox:focus-visible{outline:3px solid rgba(10,102,194,.28);outline-offset:2px}.btn-tamanho-modal:active,.extra-qtd button:active,.qtd-btn-simples:active,.btn-pedido-cta:active{transform:scale(.97)}.btn-tamanho-modal.ativo,.btn-selecionar-sabor.selecionado{box-shadow:0 0 0 3px rgba(10,102,194,.16)}.modal-extras-pontual{width:min(94vw,960px)!important;max-width:none!important;max-height:90vh!important;overflow:hidden!important}.modal-extras-pontual .modal-body{overflow-y:auto!important;min-height:0}.modal-extras-pontual #extrasContainerModal,.modal-extras-pontual #extrasSimplesContainer{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:.75rem}.produtos-grid{grid-template-columns:repeat(auto-fit,minmax(180px,1fr))!important;align-items:stretch}.produtos-grid-1{grid-template-columns:minmax(220px,420px)!important;justify-content:center!important}.produtos-grid-2{grid-template-columns:repeat(2,minmax(0,1fr))!important;max-width:760px;margin-inline:auto}.produtos-grid-muitos{grid-template-columns:repeat(auto-fit,minmax(180px,1fr))!important}.modal-extras-pontual .modal-footer,.modal-customizacao-full .modal-footer,#modalPedidoRest .modal-footer,.popup-confirmacao-footer{padding-bottom:max(.75rem,var(--reserva-barra-app,0px),env(safe-area-inset-bottom))!important}.modal-imagem-full .lado-direito{display:grid!important;grid-template-columns:minmax(2rem,auto) minmax(3rem,5rem) minmax(2rem,auto) minmax(150px,1fr)!important;align-content:start!important;align-items:center!important;justify-content:stretch!important;gap:.75rem!important;overflow-y:auto!important;padding-bottom:max(1rem,var(--reserva-barra-app,0px),env(safe-area-inset-bottom))!important}.modal-imagem-full .lado-direito>.produto-nome,.modal-imagem-full .lado-direito>.tamanho-botoes-modal,.modal-imagem-full .lado-direito>.preco,.modal-imagem-full .lado-direito>.descricao{grid-column:1/-1}.modal-imagem-full .lado-direito>.produto-quantidade-simples{grid-column:1/4;justify-content:flex-start!important;margin:0!important}.modal-imagem-full .lado-direito>#addImagem{grid-column:4;width:100%!important;margin:0!important}.modal-header .btn-modal-fechar::before{content:"\\00D7"!important}.popup-confirmacao-header .popup-confirmacao-close{position:static;background:rgba(255,255,255,.18);border-color:rgba(255,255,255,.55);color:#fff}.popup-confirmacao-header .popup-confirmacao-close:hover{background:rgba(255,255,255,.3);color:#fff}@media(max-width:700px){.modal-extras-pontual{width:calc(100vw - 1rem)!important;max-height:calc(100dvh - 1rem)!important}.modal-extras-pontual .modal-body{padding:.75rem!important}.modal-extras-pontual #extrasContainerModal,.modal-extras-pontual #extrasSimplesContainer,.produtos-grid,.produtos-grid-1,.produtos-grid-2,.produtos-grid-muitos{grid-template-columns:1fr!important;max-width:none!important}.modal-imagem-full .container-imagem{flex-direction:column!important;align-items:stretch!important}.modal-imagem-full .lado-esquerdo{height:46vh!important;min-height:180px!important;flex:0 0 46vh!important;padding:.5rem!important}.modal-imagem-full .lado-direito{display:flex!important;flex-direction:column!important;min-width:0!important;width:100%!important;height:auto!important;flex:1 1 auto!important;gap:.5rem!important;padding:.75rem!important}.modal-imagem-full .lado-direito>.produto-quantidade-simples{width:100%;justify-content:center!important}.modal-imagem-full .lado-direito>#addImagem{width:100%!important;margin-top:.25rem!important}}';
    ...
    function mostrarPopupConfirmacao(opcoes) {
        ...
        overlay.innerHTML = '<div class="popup-confirmacao-card"><div class="popup-confirmacao-header"><h3>' + opcoes.titulo + '</h3><button type="button" class="modal-close-btn popup-confirmacao-close" onclick="this.closest(\'.popup-confirmacao\').remove()" aria-label="Fechar">×</button></div><div class="popup-confirmacao-body"><p>Seu pedido foi enviado com sucesso!</p><div class="popup-confirmacao-codigo"><p class="label">Código</p><p class="valor">#' + Core.sanitize(opcoes.codigo) + '</p><button class="btn-adicionar-filtro" style="background:white;color:var(--primary);border:1px solid var(--primary);padding:0.5rem 1rem;margin-top:0.5rem;" onclick="navigator.clipboard.writeText(\'' + Core.jsEscape(opcoes.codigo) + '\').then(function(){ Economizei.UI.mostrarToast(\'Código copiado!\'); })">📋 Copiar código</button></div><div class="popup-confirmacao-botoes">' + opcoes.botoes + '</div></div><div class="popup-confirmacao-footer"><button class="btn-modal-fechar" onclick="this.closest(\'.popup-confirmacao\').remove(); ' + (opcoes.onClose || '') + '">Fechar</button></div></div>';
    ...
    function abrirModalImagemFull(produto) {
        ...
        function atualizarModal() {
            ...
            modal.querySelector('#addImagem').addEventListener('click', function() { if (produto.personalizavel === true || produto.tipo === 'personalizavel') { modal.remove(); abrirModalCustomizacaoCompleta(produto); return; } var qtd = parseInt(qtdInput.value) || 1; if (!validarEstoqueAdicao(produto, qtd)) return; var nomeCompleto = tamanhoSelecionado ? produto.nome + ' (' + tamanhoSelecionado + ')' : produto.nome; var existing = carrinho.find(function(i) { return i.id === produto.id && i.tamanho === tamanhoSelecionado; }); if (existing) existing.quantidade += qtd; else carrinho.push(Object.assign({}, produto, { id: produto.id, nome: nomeCompleto, preco: precoSelecionado, quantidade: qtd, tamanho: tamanhoSelecionado })); atualizarCarrinhoVisual(); recalcularTotal(); atualizarBadgeCarrinho(); UI.mostrarToast('Produto adicionado ao carrinho'); modal.remove(); });
        }
        atualizarModal(); document.body.appendChild(modal); UI.trapFocus(modal);
    }

    function abrirModalCustomizacaoCompleta(produto) {
        var tamanhos = produto.tamanhosDisponiveis || []; var temTamanhosCustom = tamanhos.length > 0; if (temTamanhosCustom) { tamanhoSelecionadoCustom = tamanhos[0].nome; } else { tamanhoSelecionadoCustom = null; }
        limiteSaboresPorTamanho = {}; tamanhos.forEach(function(t) { limiteSaboresPorTamanho[t.nome] = t.maxSabores || 1; }); if (!temTamanhosCustom) limiteSaboresPorTamanho.__sem_tamanho__ = produto.maxSabores || 1;
        ...
        modal.innerHTML = '<div class="modal-conteudo modal-customizacao-full"><div class="modal-header"><h3>' + Core.sanitize(produto.nome) + '</h3><button class="btn-modal-fechar" onclick="this.closest(\'.modal-overlay\').remove()" aria-label="Fechar">✕</button></div><div class="modal-body"><div class="customizacao-layout"><div class="customizacao-imagem">' + (produto.imagem ? '<img src="' + produto.imagem + '" alt="' + Core.sanitize(produto.nome) + '" onclick="this.closest(\'.modal-overlay\').remove(); Economizei.Pedido.abrirImagemProduto(\'' + Core.jsEscape(produto.id) + '\', true)" style="cursor:pointer;">' : '<div style="width:100%; max-width:300px; background:#eee; border-radius:1rem; padding:2rem; text-align:center;">📷</div>') + '</div><div class="customizacao-opcoes"><div class="descricao-produto">' + Core.sanitize(produto.descricao || 'Sem descrição') + '</div><div id="precoBaseMsg" style="font-size:0.9rem; color:var(--gray-700); margin-bottom:0.5rem;">Escolha os sabores para calcular o preço</div><div class="tamanho-botoes-modal" id="customTamanhos">' + (temTamanhosCustom ? tamanhos.map(function(t) { return '<button class="btn-tamanho-modal ' + (t.nome === tamanhoSelecionadoCustom ? 'ativo' : '') + '" data-tamanho="' + Core.sanitize(t.nome) + '">' + Core.sanitize(t.nome) + '</button>'; }).join('') : '<span style="font-size:.8rem;color:var(--gray-500);">Tamanho não se aplica a este produto.</span>') + '</div><div class="campo"><div style="display:flex; justify-content:space-between; align-items:center;"><label>Sabores (máx: <span id="limiteSaboresLabel">' + (limiteSaboresPorTamanho[tamanhoSelecionadoCustom || '__sem_tamanho__'] || 1) + '</span>)</label><button class="btn-expandir-categoria" id="btnExpandirSabores" style="background:none; border:none; font-size:1rem; cursor:pointer;" aria-label="Expandir sabores">[−]</button></div><div id="saboresLista" class="lista-sabores" style="display:block;"></div></div><div class="campo extras-group"><div style="display:flex; justify-content:space-between; align-items:center;"><label>Extras disponíveis</label><button class="btn-expandir-categoria" id="btnExpandirExtras" style="background:none; border:none; font-size:1rem; cursor:pointer;" aria-label="Expandir extras">[−]</button></div><div id="extrasLista" class="lista-extras" style="display:block;"></div></div><div class="preco-total-custom" id="precoTotalCustom">Total: \u2014</div></div></div></div><div class="modal-footer"><button class="btn-pedido-cta" id="btnAddCustomizado">Adicionar ao carrinho</button></div></div>';
    ...
    function abrirModalExtrasSimples(produto) {
        ...
        modal.innerHTML = '<div class="modal-conteudo modal-extras-pontual"><div class="modal-header"><h3>' + Core.sanitize(produto.nome) + '</h3><button class="btn-modal-fechar" onclick="this.closest(\'.modal-overlay\').remove()" aria-label="Fechar">✕</button></div><div class="modal-body"><div style="margin-bottom:0.5rem;">Preço: R$ ' + precoBase.toFixed(2) + '</div><div><strong>Extras</strong></div><div id="extrasSimplesContainer" style="margin-top:0.5rem;">' + (selecaoExtras.length === 0 ? '<p style="text-align:center;color:var(--gray-500);">Nenhum extra disponível.</p>' : '') + '</div><div id="precoTotalSimples" class="preco-total-custom" style="margin-top:1rem;">Total: R$ ' + precoBase.toFixed(2) + '</div></div><div class="modal-footer"><button class="btn-pedido-cta" id="btnAddExtraSimples">Adicionar ao carrinho</button></div></div>';
        document.body.appendChild(modal); UI.trapFocus(modal);
        var extrasContainer = modal.querySelector('#extrasSimplesContainer');
        var precoTotalEl = modal.querySelector('#precoTotalSimples');
        function renderizarExtras() { extrasContainer.innerHTML = ''; if (selecaoExtras.length === 0) { extrasContainer.innerHTML = '<p style="text-align:center;color:var(--gray-500);">Nenhum extra disponível.</p>'; return; } selecaoExtras.forEach(function(extra) { var extraDiv = document.createElement('div'); extraDiv.className = 'item-extra'; var header = document.createElement('div'); header.className = 'item-header'; header.style.display = 'flex'; header.style.alignItems = 'center'; header.style.justifyContent = 'space-between'; if (extra.imagem) { var img = document.createElement('img'); img.src = extra.imagem; img.className = 'extra-imagem'; img.alt = ''; header.appendChild(img); } var nomeSpan = document.createElement('span'); nomeSpan.className = 'item-nome'; nomeSpan.textContent = extra.nome; var precoSpan = document.createElement('span'); precoSpan.className = 'item-preco'; precoSpan.textContent = 'R$ ' + extra.preco.toFixed(2); var infoDiv = document.createElement('div'); infoDiv.style.display = 'flex'; infoDiv.style.alignItems = 'center'; infoDiv.style.gap = '0.5rem'; infoDiv.style.flex = '1'; infoDiv.appendChild(nomeSpan); infoDiv.appendChild(precoSpan); header.appendChild(infoDiv); var descDiv = document.createElement('div'); descDiv.className = 'item-descricao'; descDiv.textContent = extra.descricao || 'Sem descrição'; var qtdDiv = document.createElement('div'); qtdDiv.style.padding = '0'; qtdDiv.style.marginLeft = 'auto'; qtdDiv.innerHTML = '<div class="extra-qtd"><button data-extra-id="' + extra.id + '" data-delta="-1" aria-label="Diminuir quantidade de ' + Core.sanitize(extra.nome) + '">−</button><span id="extra-simples-qtd-' + extra.id + '">' + extra.quantidade + '</span><button data-extra-id="' + extra.id + '" data-delta="1" aria-label="Aumentar quantidade de ' + Core.sanitize(extra.nome) + '">+</button></div>'; header.appendChild(qtdDiv); extraDiv.appendChild(header); extraDiv.appendChild(descDiv); extrasContainer.appendChild(extraDiv); var qtdButtons = qtdDiv.querySelectorAll('button[data-delta]'); qtdButtons.forEach(function(btn) { btn.addEventListener('click', function(e) { e.stopPropagation(); var extraId = btn.dataset.extraId; var delta = parseInt(btn.dataset.delta); var extraItem = selecaoExtras.find(function(ex) { return ex.id === extraId; }); if (extraItem) { var novaQtd = extraItem.quantidade + delta; if (novaQtd < 0) novaQtd = 0; if (extraItem.max && novaQtd > extraItem.max) novaQtd = extraItem.max; extraItem.quantidade = novaQtd; var span = qtdDiv.querySelector('#extra-simples-qtd-' + extraId); if (span) span.textContent = novaQtd; atualizarPrecoTotalSimples(); } }); }); }); }
    ...
    function alterarQuantidade(id, value) { var qtd = parseInt(value) || 1; var item = carrinho.find(function(i) { return i.id === id; }); if (item) { if (item.estoque !== null && item.estoque !== undefined && item.estoque !== '') { var estoque = parseInt(item.estoque); var outros = carrinho.reduce(function(total, i) { return total + (i !== item && i.id === id ? (parseInt(i.quantidade, 10) || 0) : 0); }, 0); if (!isNaN(estoque) && outros + qtd > estoque) { UI.mostrarToast('Estoque insuficiente. Disponível: ' + estoque); atualizarCarrinhoVisual(); return; } } item.quantidade = Math.max(1, qtd); atualizarCarrinhoVisual(); recalcularTotal(); atualizarBadgeCarrinho(); } }
    ...
    function renderizarProdutos(produtos) { if (!produtos || produtos.length === 0) return '<p style="text-align:center;padding:2rem;">Nenhum produto disponível.</p>'; var categorias = {}; produtos.forEach(function(p) { var cat = p.categoria || 'Geral'; if (!categorias[cat]) categorias[cat] = []; categorias[cat].push(p); }); var html = ''; for (var cat in categorias) { var quantidadeCategoria = categorias[cat].length; var classeCategoria = quantidadeCategoria === 1 ? ' produtos-grid-1' : quantidadeCategoria === 2 ? ' produtos-grid-2' : ' produtos-grid-muitos'; html += '<div class="categoria-group"><div class="categoria-titulo-modal" onclick="this.nextElementSibling.style.display = this.nextElementSibling.style.display === \'none\' ? \'grid\' : \'none\';" role="button" tabindex="0" aria-expanded="true">' + Core.sanitize(cat) + ' ▼</div><div class="produtos-grid' + classeCategoria + '" style="display:grid;">'; categorias[cat].forEach(function(prod) { html += gerarHTMLProduto(prod); }); html += '</div></div>'; } return html; }
